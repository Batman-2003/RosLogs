...
def curviliner_path_calculator(radius, centroid_x, offset):
    x = np.linspace(centroid_x - radius + offset, centroid_x + radius + offset, 60)
    z = np.zeros(60)
    for _,x_val in enumerate(x):
        z[_] = m.sqrt( (radius + offset)**2 - (x[_] - centroid_x)**2) + radius - 1.1
...
##----------------------------------------------------------------------Main Func---------------------------------------------------------------
# wb = openpyxl.load_workbook("../my_ws/src/collision_1/data_collection/test.xlsx")                         ##Modifying the path for roslaunch 
...
if __name__ == '__main__':
    ...
    y = 0.1
    x,z = curviliner_path_calculator(paras[5], paras[8], 0.02)
    print("radius = {r} and x_offset {x}".format(r = paras[5] , x = paras[8]))
